...
import { supabaseClient } from '@/utils/customPDFLoader'
import { databaseIds } from '@/config/notionIds'

async function extractDataFromNotionDatabase(notionApiKey: string, databaseId: string): Promise<Document[]> {
  try {
    const loader = new CustomNotionLoader(notionApiKey, databaseId)
    const docs = await loader.load()
    return docs
  } catch (error) {
    console.error(`Error while extracting data from database ${databaseId}: ${error}`)
    return []
  }
}

async function extractDataFromNotionDatabases(notionApiKey: string, databaseIds: string[]): Promise<Document[]> {
...
async function embedDocuments(client: SupabaseClient, docs: Document[], embeddings: Embeddings) {
...
;(async function run(notionApiKey: string, databaseIds: string[]) {
  try {
    ...
    const rawDocs = await extractDataFromNotionDatabases(notionApiKey, databaseIds)
...
})('secret_zLf9KlKyLjDkz1OOPQBCNIA63XXkIYNgyxPcsROVBwM', databaseIds)
