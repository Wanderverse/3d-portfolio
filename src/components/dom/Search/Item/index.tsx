      ...
      {item.list.map((x: any) => (
        ...
        >
          <div className='relative shrink-0 w-12 h-12'>
            <Image className='rounded-full object-cover' src={x.avatar} fill alt='Avatar' />
            {x.online && (
          ...
          <div className='w-[calc(100%-3rem)] pl-5'>
            <div className='mb-1 truncate base1 font-semibold'>{x.title}</div>
            <div className='truncate caption1 text-n-4/75'>{x.content}</div>
          </div>
          ...
          <div className='absolute top-1/2 -translate-y-1/2 left-full flex items-center ml-9 px-3 py-2 bg-n-7 rounded-lg whitespace-nowrap text-n-4 base2 invisible opacity-0 transition-all group-hover:visible group-hover:opacity-100 2xl:hidden dark:bg-n-2'>
            ...
            <span className='mr-2 font-semibold text-n-1 dark:text-n-7'>{x.author}</span>
            {x.time}
          </div>
        </Link>
      ))}
