    ...
    <Listbox value={value} onChange={onChange}>
      {({ open }) => (
        <>
          ...
          >
            {title && <div className='shrink-0 mr-2 pr-2 border-r border-n-3 text-n-4 dark:border-n-4/50'>{title}</div>}
            {icon && (
              <Icon className={`shrink-0 mr-2 dark:fill-n-4 ${small && 'w-5 h-5 mr-1.5'} ${classIcon}`} name={icon} />
            )}
            {value?.color && (
              <div className='shrink-0 w-3.5 h-3.5 ml-1 mr-4 rounded' style={{ backgroundColor: value.color }}></div>
            )}
            ...
            <Icon
              className={`shrink-0 ml-2 transition-transform dark:fill-n-1 ${open && 'rotate-180'} ${
                small && 'ml-1'
              } ${classArrow}`}
              name='arrow-down'
            />
          ...
          <Transition leave='transition ease-in duration-100' leaveFrom='opacity-100' leaveTo='opacity-0'>
            <Listbox.Options
              className={twMerge(
                ...
                } ${up && `top-auto bottom-full mt-0 ${small ? 'mb-1' : 'mb-2'}`} ${open && 'z-10'} ${classOptions}`,
              )}
            >
              {items.map((item: any) => (
                <Listbox.Option
